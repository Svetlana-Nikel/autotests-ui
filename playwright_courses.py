...
with sync_playwright() as playwright:
    ...
    expect(courses_title).to_be_visible() #проверяем наличие заголовка
    expect(courses_title).to_have_text('Courses') #проверяем текст заголовка

    empty_view_icon = page.get_by_test_id('courses-list-empty-view-icon')
    expect(empty_view_icon).to_be_visible() #проверяем видимость иконки
